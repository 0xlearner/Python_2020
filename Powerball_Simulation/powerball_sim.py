...
for i in range(5):
    ...
    #(69*68*67*66*65)*26/120 normal powerball
...
#Generating the winning lottery numbers
...
#Run the lottery if we haven't purchased the winning ticket and we still we want to play
while winning_numbers not in tickets_sold and active == True:
    #Make new lottery ticket for the user to buy
    lottery_numbers = []
    #Get the white-balls for the ticekt
    ...
    if tickets_purcahsed % ticket_interval == 0:
        print(f'{tickets_purcahsed} tickets purchased so far with no winners...')
        choice = input('\nKeep purchasing tickets[y/n]: ')
        if choice != 'y':
            active = False
#The lottery is over now
#We purchased the winning ticket and we won the lottery
if lottery_numbers == winning_numbers:
    print('\nWinning ticket numbers: ', end="")
    for number in lottery_numbers:
        print(f'{number}', end=' ')
    print(f"\nPurchased a total of {tickets_purcahsed} tickets.")
#We didn't purchase the winning ticket and we gave up
else:
    print(f"\nYou bought {tickets_purcahsed} tickets and still lost!")
